...
namespace mabe {
  ...
  template <typename T> class BasicBrain : public Brain { // returns whatever's in output
    public:
        ...
        emp::Ptr<Brain> Clone() override {
            return emp::NewPtr<BasicBrain<T>>(*this); 
        }
  };

  // remember to put this in a different file later
  class BasicBrainHead : public Brain { // returns a head of the first genome
    public:
        BasicBrainHead() {};
        BasicBrainHead(const BasicBrainHead &) = default;
        void process(emp::DataMap org_map) override {
            emp::vector<emp::Ptr<Genome>> genomes = org_map.Get<emp::vector<emp::Ptr<Genome>>>("genomes");
            //org_map.Get<emp::Ptr<Organism>>("organism")->SetVar<Genome::Head>(org_map.Get<std::string>("output_name"), Genome::Head(*(genomes[0])));
            SetVar<Genome::Head>(org_map.Get<emp::Ptr<emp::DataMap>>("data_map"), org_map.Get<std::string>("output_name"), Genome::Head(*(genomes[0])));
