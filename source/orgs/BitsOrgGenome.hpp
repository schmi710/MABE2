 ...
 *  @date 2019-2020.
 *
 *  @file  BitsOrgGenome.hpp
 *  @brief An organism consisting of a series of bits.  Uses new Genome class.
 *  @note Status: ALPHA
 */

#ifndef MABE_BITS_GENOME_ORGANISM_H
#define MABE_BITS_GENOME_ORGANISM_H
...
namespace mabe {
  ...
  class BitsOrgGenome : public OrganismTemplate<BitsOrgGenome> {
  protected:
    TypedGenome<bool> genome;

  public:
    BitsOrgGenome(OrganismManager<BitsOrgGenome> & _manager)
      : OrganismTemplate<BitsOrgGenome>(_manager), genome(100) { }
    BitsOrgGenome(const BitsOrgGenome &) = default;
    BitsOrgGenome(BitsOrgGenome &&) = default;
    BitsOrgGenome(const TypedGenome<bool> & in, OrganismManager<BitsOrgGenome> & _manager)
      : OrganismTemplate<BitsOrgGenome>(_manager), genome(in) { }
    BitsOrgGenome(size_t N, OrganismManager<BitsOrgGenome> & _manager)
      : OrganismTemplate<BitsOrgGenome>(_manager), genome(N) { }
    ~BitsOrgGenome() { ; }
  ...
  MABE_REGISTER_ORG_TYPE(BitsOrgGenome, "Organism consisting of a series of N bits. Uses new Genome class.");
}
