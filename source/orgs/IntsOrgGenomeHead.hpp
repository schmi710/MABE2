 ...
 *  @date 2019-2020.
 *
 *  @file  IntsOrgGenomeHead.hpp
 *  @brief An organism consisting of a series of ints.  Uses new Genome class and the head.
 *  @note Status: ALPHA
 */

#ifndef MABE_INTS_GENOME_HEAD_ORGANISM_H
#define MABE_INTS_GENOME_HEAD_ORGANISM_H
...
namespace mabe {
  ...
  class IntsOrgGenomeHead : public OrganismTemplate<IntsOrgGenomeHead> {
  protected:
    TypedGenome<int> genome;

  public:
    IntsOrgGenomeHead(OrganismManager<IntsOrgGenomeHead> & _manager)
      : OrganismTemplate<IntsOrgGenomeHead>(_manager), genome(100) { }
    IntsOrgGenomeHead(const IntsOrgGenomeHead &) = default;
    IntsOrgGenomeHead(IntsOrgGenomeHead &&) = default;
    IntsOrgGenomeHead(const TypedGenome<int> & in, OrganismManager<IntsOrgGenomeHead> & _manager)
      : OrganismTemplate<IntsOrgGenomeHead>(_manager), genome(in) { }
    IntsOrgGenomeHead(size_t N, OrganismManager<IntsOrgGenomeHead> & _manager)
      : OrganismTemplate<IntsOrgGenomeHead>(_manager), genome(N) { }
    ~IntsOrgGenomeHead() { ; }
    ...
    struct ManagerData : public Organism::ManagerData {
      ...
      double max_value = INT_MAX+1.0;           ///< The maximum integer value.
    };

    /// Use "to_string" to convert.
    std::string ToString() const override { return "using ints: " + emp::to_string(genome); }

    size_t Mutate(emp::Random & random) override { 
      genome.SetTheDeets(SharedData().mut_prob, 0, SharedData().max_value);
    ...
    void SetupConfig() override {
      ...
      GetManager().LinkVar(SharedData().output_name, "output_name",
                      "Name of variable to contain int sequence.");
    ...
    void SetupModule() override {
      ...
      GetManager().AddSharedTrait(SharedData().output_name,
                                  "Int output from organism.",
                                  Genome::Head(genome));
    }
  };

  MABE_REGISTER_ORG_TYPE(IntsOrgGenomeHead, "Organism consisting of a series of N ints. Uses new Genome class and the head.");
}
